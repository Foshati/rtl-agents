...
*   **Multi-IDE Support**: Automatically detects and patches the active IDE resources (VS Code, Cursor, Windsurf, VSCodium, or Antigravity).
...
*   **Code Block & Layout Safety**: Keeps code syntax blocks (`pre`, `code`), Monaco editors, data tables, SVG icons, and action buttons in LTR, ensuring your coding workspace is never broken.
...
*   **Custom Selectors & Configuration**: Customize target CSS selectors and the RTL character detection regex patterns directly in VS Code settings.
...
Because this extension modifies the workbench UI elements on disk, it requires a simple, one-time activation.
...
*   `RTL Agents: Toggle RTL/LTR` (Shortcut: `Ctrl+Alt+R` / `Cmd+Ctrl+R`): Toggles the active status on the fly.
*   `RTL Agents: Check Status`: Displays the patch status of all installations in an output channel.
*   `RTL Agents: Restart RTL`: Restores original files first, then re-applies the patch (useful if files are out of sync).
